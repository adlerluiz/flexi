		...
		<q-tab-panels v-model="tab">
			<q-tab-panel name="runner">
				<div v-for="(objWindow, index) in appList" :key="index">
					<div
						class="row q-pa-sm q-gutter-x-md items-centera justify-around"
					>
						<div class="col-auto">
							<q-btn
								@click="
								...
								"
								:color="
									checkIsRunning(objWindow.id) ? 'red-8' : 'green-8'
								"
								:icon="
									checkIsRunning(objWindow.id) ? 'stop' : 'play_arrow'
								"
						...
						<div class="col-3">
							<q-select
								...
								@update:model-value="
									(val) => handleDeviceChange(index, val)
								"
								:options="deviceList"
								option-label="name"
							/>
					...
					>
						...
						<div class="col-auto">
							<q-btn
								...
								color="grey-8"
								:icon="
									device.isVisible ? 'visibility' : 'visibility_off'
								"
...
export default defineComponent({
	...
	mounted() {
		...
		window.electron.on('snapshot-saved', (event: any, data: any) => {
			alert(`Saved on ${data}`);
		});
